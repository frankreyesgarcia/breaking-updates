...
public class Attempt {
    ...
    public void dockerMetadata(Path benchmarkDir) {
        ...
        Path reproducibilityResultsFilePath = Path.of("results-10-attempts" + JsonUtils.JSON_FILE_ENDING);
        ...
        Map<String, Map<String, Long>> results = JsonUtils.readFromNullableFile(reproducibilityResultsFilePath, dockerData);
        if (results == null) {
            results = new HashMap<>();
        }
...
//            for (File breakingUpdate : breakingUpdates) {
            for (int i = 1; i <= 10; i++) {
                File breakingUpdate = breakingUpdates[0];
                ...
                if (!results.containsKey((String) bu.get("breakingCommit"))) {
                    ...
                    Path logDirectory;
                    try {
                        logDirectory = Files.createDirectories(Path.of("executionTimeLogs-attempt").resolve((String) bu.get("breakingCommit")+ "attempt "+ i));
                    } catch (IOException e) {
                    ...
                    results.put((String) bu.get("breakingCommit")+"attempt "+ i, imageMetadata);
    ...
    private Map.Entry<String, ImageData> startContainer(String image, boolean isPrevImage, String project, Path logDirectory) {
        ...
        CreateContainerCmd containerCmd = dockerClient.createContainerCmd(image).withWorkingDir("/" + project).withCmd("sh", "-c", "--network none", "set -o pipefail && (mvn clean test -B 2>&1 | tee -ai output.log)");
    ...
    private void createDockerClient() {
        DockerClientConfig clientConfig = DefaultDockerClientConfig.createDefaultConfigBuilder().withRegistryUrl("https://hub.docker.com").build();
        DockerHttpClient httpClient = new OkDockerHttpClient.Builder().dockerHost(clientConfig.getDockerHost()).sslConfig(clientConfig.getSSLConfig()).connectTimeout(30).build();
